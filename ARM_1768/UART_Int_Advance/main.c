...
char buf[64] = { 0 };//64 Byte Buffer
int rxWriter = 0;
int rxReader = 0;
char* readData(char *buf, int len);
char readChar();
...
void uartInit3()
{
	...
	*/
	...
	BIT[0] RBR InterruptEnable		Enables the Receive Data Available interrupt for UARTn. It also controls
...
{
	/*
	UARTn Interrupt Identification Register (U3IIR)
	BIT[3:1]		IntId				010			 2a - Receive Data Available (RDA).
	*/
	if(LPC_UART3->IIR & (1<<2))//Check Which INT FLAG
	{
		buf[rxWriter] = LPC_UART3->RBR;
		if(buf[rxWriter] != 0)
		{
			rxWriter++;
			if(rxWriter >= 64)
...
char* readData(char *buf, int len)
{
    char temp = 0;
    int index = 0;
    while(((temp = readChar()) != 0x0D) && (index<len))
        {
            //sendChar(temp);//Echo
					if(buf[index] != 0)
					{
